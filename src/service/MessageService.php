...
class MessageService extends Service
{
     ...
     */
    public function sendVerifyCode(string $phone, int $wait = 60, string $tplcode = 'zt.tplcode_register|raw'): array
    {
        ...
        // 尝试发送短信内容
        [$state] = $this->send($phone, $code, preg_replace_callback("|{(.*?)}|", function ($matches) use ($code) {
     ...
     * @return array
     */
    public function send(string $phone, int $code, string $content): array
    {
        [$state, $message, $record] = $this->_request('v2/sendSms', ['mobile' => $phone, 'content' => $content]);
        SysSms::mk()->insert(['phone' => $phone, 'code' => $code, 'content' => $content, 'result' => $message, 'is_deleted' => 0, 'status' => $state ? 1 : 0]);
     ...
     * @return array
     */
    public function sendNotify(string $phone, string $tplcode = '【双擎码】您已成功支付，请等待平台分配通用码到您的账户。'): array
    {
        $content = $tplcode ? $tplcode : '【双擎码】您已成功支付，请等待平台分配通用码到您的账户。';
        // 生成新的验证码
        $code = rand(1000, 9999);
        // 尝试发送短信内容
        [$state] = $this->send($phone, $code, $content);
        if ($state) {
            return [1, '短信发送成功！'];
        } else {
            return [0, '短信发送失败，请稍候再试！'];
    ...
    {
        $this->username = syconfig('ZT_SMS', 'username');
        $this->password = syconfig('ZT_SMS', 'password');
        return $this;
    }
